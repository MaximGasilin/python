# Задание # 2
#
# Представлен список чисел. Необходимо вывести элементы исходного списка, значения
# которых больше предыдущего элемента.
# Подсказка: элементы, удовлетворяющие условию, оформить в виде списка. Для
# формирования списка использовать генератор.
# Пример исходного списка: [300, 2, 12, 44, 1, 1, 4, 10, 7, 1, 78, 123, 55].
# Результат: [12, 44, 4, 10, 78, 123].


# Для генерации списка случайных чисел нужно подключить модуль random
from random import randint

# my_list = [300, 2, 12, 44, 1, 1, 4, 10, 7, 1, 78, 123, 55]
my_list = [randint(-100, 100) for i in range(20)]
print(my_list)

# Технический первый элемент, чтобы дальнейший алгоритм выглядел красиво. Конечно, этот элемент будет больше второго.
my_list.insert(0, my_list[0]+1)
new_list = [my_list[i] for i in range(1, len(my_list)) if my_list[i - 1] < my_list[i]]

print(new_list)
